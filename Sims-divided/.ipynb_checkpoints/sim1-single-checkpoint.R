...
out_filename <- paste0("sim1-keele-overlap", overlap_degree, ".txt")
...
run_scenario = function( c ) {
  ...
  reps_qs0 = map( 1:sim_reps, function( id ) {
    if (id <= 10) cat(paste("Starting simulation", id, "for overlap", c, "at", Sys.time(), "\n"), file = out_filename, append = TRUE)
...
filename <- paste0("simulation-1-keele-", sim_reps, "-overlap", overlap_degree, ".RData")
