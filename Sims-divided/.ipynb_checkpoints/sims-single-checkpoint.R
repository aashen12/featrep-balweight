...
out_filename <- paste0("logs/sim", sim_num, "-keele-", overlap_degree, ".txt")
...
if (sim_num == 1) {
  make_data <- function(n,c, treat.true){
    ...
    Y0 <- 2*X1 + X2 + X3 + rnorm(n) # 2X1 is new
    Y1 <- Y0 + treat.true
    Y <- Z*Y1 + (1 - Z)*Y0
  ...
  make_data <- function(n,c, treat.true){
    ...
    Z <- as.numeric((z.star / c + runif(n) - 0.5) > 0)
    Y0 <- (X1 + X2 + X3 + X4 + X5)^3 + mvrnorm(n=1, mu=rep(0, nrow(C)), Sigma=diag(nrow(C))) # X4 + X5 is new
    Y1 <- Y0 + treat.true
    Y <- Z*Y1 + (1 - Z)*Y0
    simDat <- data.frame(cbind(Z, Y, X1, X2, X3, X4, X5))
    return(simDat)
  ...
  make_data <- function(n,c, treat.true){
    ...
    z.star <- 1.5*X1^3 + 1.5*X2 - 2*X3^3 + X4 + 0.5*X5
    print(head(z.star))
    Z <- as.numeric((z.star / c + runif(n) - 0.5) > 0)
...
run_scenario = function( c ) {
  ...
  reps_qs0 = map( 1:sim_reps, function( id ) {
    ...
    pilot.dat <- make_data(1000, c=c, treat.true=5 ) %>% dplyr::filter(Z == 0)
...
save(scenarios, file="simulations-keele-temp.RData")

filename <- paste0("results/simulation-", sim_num, "-keele-", sim_reps, "-overlap", overlap_degree, ".RData")
